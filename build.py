...
hbb_name = 'rahbardesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/rahbardesk.exe')
    os.chdir('../..')
    if os.path.exists('./rahbardesk_portable.exe'):
        os.replace('./target/release/rahbardesk-portable-packer.exe',
                   './rahbardesk_portable.exe')
    else:
        os.rename('./target/release/rahbardesk-portable-packer.exe',
                  './rahbardesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/rahbardesk_portable.exe')
    os.rename('./rahbardesk_portable.exe', f'./rahbardesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/rahbardesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rahbardesk.exe target/release/RahbarDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\rahbardesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/RahbarDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
